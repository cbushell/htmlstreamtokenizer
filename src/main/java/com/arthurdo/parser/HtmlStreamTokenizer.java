 ...
 * class but is specialized for HTML streams. This class is useful when you need
 * to parse the structure of an HTML document.
 * </p>
 * 
 * <pre>
 ...
 * 
 * <p>
 * One of the motivations for designing <i>parseTag()</i> to take an HtmlTag
 * argument rather than having <i>parseTag()</i> return a newly created HtmlTag
 * is so you can create your own tag class derived from HtmlTag.
 * 
 * <ul>
 * <li>02/09/98 Thomas Horster-Möller, fixed bug with counting newlines twice on
 * character pushback.
 * <li>06/14/99 text is now returned as 'runs' instead of being broken up into
 * words as in previous versions. You can use a StringTokenizer to break your
 * text into words.
 * </ul>
 * 
...
public class HtmlStreamTokenizer {
	...
	/**
	 * inside <! to provide support for doctypes with internal dtd, <![CDATA
	 * sections, and degenerate html comments
	 */
	...
	/**
	 * @param in
	 *            Reader. The input is assumed to be buffered as needed.
	 */
	...
	/**
	 * @return string value of the token, including characters stripped off by
	 *         the tokenizer
	 */
	...
	/**
	 * @return current line number. Every time nextToken() sees a new line
	...
	/**
	 * @param char[] exitString CDATA mode will terminate when it encounters
	 *        this string
	 * @param boolean pushbackExitString whether to parse the exit string again
	 *        or not
	 * 
	 *        it'd be an error to call enterCDATAMode(exitString, true);
	 *        getToken()==TT_CDATA; enterCDATAMode(differentExitString, true);
	 *        'cause the next getToken() call will parse differentExitString
	 *        instead of exitString
	 */
			...
			case STATE_TEXT: {
				if (m_cdata > -1) {
					...
					if ((Character.toUpperCase(m_cdata_end[m_cdata]) == c)
							|| (Character.toLowerCase(m_cdata_end[m_cdata]) == c)) // support
																					// case
																					// sensitive
																					// exit
																					// strings
				...
				/*
				 * else if (isSpace(c)) { m_pushback = c; m_state = STATE_WS; if
				 * (m_buf.length() > 0) { if (m_unescape && hasAmp)
				 * unescape(m_buf); return m_ttype = TT_TEXT; } }
				 */
				else {
					if (c == '&') {
						if (m_getEntities) {
	...
	private static Escapes escapes = new Escapes();
